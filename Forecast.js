...
for(let i = 0 ; i < result.list.length ; i++){
    ...
    Temperature.textContent = result.list[i].main.temp;
    newDataDiv.appendChild(Temperature);
    let  weather= document.createElement("h2");
    weather.textContent = result.list[i].weather[0].main;
    newDataDiv.appendChild(weather);
    let image= document.createElement("img");
    if(result.list[i].weather[0].main=="Clouds"){
        image.src = "./Cloudpng.png";
    }
    else if(result.list[i].weather[0].main=="Clear"){
        image.src = "./sunpng.png";
    }
    else if(result.list[i].weather[0].main=="Rain"){
        image.src = "./Rainpng.png";
    }
    image.setAttribute('id', 'img');
    newDataDiv.appendChild(image);
    let wind = document.createElement("h2");
    wind.textContent = result.list[i].wind.speed;
    newDataDiv.appendChild(wind);
    let time = document.createElement("h3");
    const weekday = ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];
    const d = new Date(result.list[i].dt_txt);
    let day = weekday[d.getDay()];
    time.textContent = day;
    newDataDiv.appendChild(time);
    let place = document.createElement("h3");
    place.textContent = result.city.name;
    newDataDiv.appendChild(place);
    document.getElementById("data").appendChild(newDataDiv);
    i = i+7;
